...
* Check the `ConnectionStrings` in `appsettings.json` files under the `InternshipManagementSystem.HttpApi.Host` and `InternshipManagementSystem.DbMigrator` projects and change it if you need.
...
Run `InternshipManagementSystem.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.

### Solution structure

This is a layered monolith application that consists of the following applications:

* `InternshipManagementSystem.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
* `InternshipManagementSystem.HttpApi.Host`: ASP.NET Core API application that is used to expose the APIs to the clients.
